...
describe 1 do
  describe 'when doing a memory decode' do
    ...
    it 'must read corrupted memory from file' do
      ...
      _(corrupted_data).must_equal 'xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))'
    end

    it 'processes input and store the output as 2d array' do
      output = process_input('xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))')
      _(output).must_equal [[2, 4], [5, 5], [11, 8], [8, 5]]
    end

    it 'iterates over each element to determine the product of each' do
      input = [[2, 4], [5, 5], [11, 8], [8, 5]]
      _(calculate_product(input)).must_equal [8, 25, 88, 40]
    end

    it 'calculates the sum of each multiplication product' do
      input = [8, 25, 88, 40]
      _(calculate_sum(input)).must_equal 161
    end
  end
end
